...
import { ENTITY_REQUIRED_FIELDS, TOOL_REQUIRED_FIELDS } from '../../mcp/schemas/contracts.ts';
...
test('AT-002 tool schema inventory exists', () => {
  for (const required of ['team_start.schema.json', 'team_spawn.schema.json', 'team_spawn_ready_roles.schema.json', 'team_send.schema.json', 'team_broadcast.schema.json', 'team_task_next.schema.json', 'team_task_cancel_others.schema.json']) {
    assert.ok(toolSchemas[required], `missing ${required}`);
  }
});

test('AT-002 TS contract required-field map matches entity schemas', () => {
  for (const [schemaName, requiredFields] of Object.entries(ENTITY_REQUIRED_FIELDS)) {
    const schema = entitySchemas[schemaName];
    assert.ok(schema, `missing entity schema: ${schemaName}`);
    const actual = [...(schema.required ?? [])].sort();
    const expected = [...requiredFields].sort();
    assert.deepEqual(actual, expected, `entity required mismatch for ${schemaName}`);
  }
});

test('AT-002 TS contract required-field map matches tool schemas', () => {
  for (const [schemaName, requiredFields] of Object.entries(TOOL_REQUIRED_FIELDS)) {
    const schema = toolSchemas[schemaName];
    assert.ok(schema, `missing tool schema: ${schemaName}`);
    const actual = [...(schema.required ?? [])].sort();
    const expected = [...requiredFields].sort();
    assert.deepEqual(actual, expected, `tool required mismatch for ${schemaName}`);
  }
});
